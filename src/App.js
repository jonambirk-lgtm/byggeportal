...
export default function App() {
  ...
  useEffect(()=>{
    ...
    supabase.from('events').select('*, event_attendees(user_id)').order('event_date',{ascending:true}).then(({data})=>{ if(data) setEvents(data.map(e=>({...e,date:e.event_date,time:e.event_time,desc:e.description,type:e.type||'Andet',attendees:(e.event_attendees||[]).map(a=>a.user_id)}))); });
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <Btn accent={acc} style={{flex:2}} onClick={async()=>{
            ...
            const {data}=await supabase.from('events').insert({
              title:f.title, event_date:f.date, event_time:f.time,
              location:f.location, description:f.desc, created_by:user.id, type:f.type
            }).select().single();
            if(data) setEvents(e=>[{...data,date:data.event_date,time:data.event_time,desc:data.description,type:data.type,attendees:[]},...e]);
